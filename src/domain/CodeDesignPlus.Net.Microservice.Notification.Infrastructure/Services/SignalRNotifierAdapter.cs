...
{
    public async Task SendToUserAsync(string userId, string method, string payload, CancellationToken ct)
    {
        // TODO: Asegurar que el 'userId' coincide con el Claim 'sub' o UserIdentifier del JWT del frontend
        await hubContext.Clients.User(userId).SendAsync(method, payload, cancellationToken: ct);
    }
