...
vault kv put -mount=security-codedesignplus-keyvalue ms-notification \
...
vault kv get -mount=security-codedesignplus-keyvalue ms-notification

# Write database configuration
echo "$newlines"
echo "${BLUE}7. Writing database configuration...${NC}"
vault write security-codedesignplus-database/config/db-ms-notification \
    plugin_name=mongodb-database-plugin \
    allowed_roles="ms-notification-mongo-role" \
    connection_url="mongodb://{{username}}:{{password}}@mongo:27017/admin?ssl=false" \
    username="admin" \
    password="password"

vault write security-codedesignplus-database/roles/ms-notification-mongo-role \
    db_name=db-ms-notification \
    creation_statements='{ "db": "admin", "roles": [{ "role": "readWrite", "db": "db-ms-notification" }] }' \
    default_ttl="1h" \
    max_ttl="24h"

vault read security-codedesignplus-database/creds/ms-notification-mongo-role
...
vault write security-codedesignplus-rabbitmq/roles/ms-notification-rabbitmq-role \
    vhosts='{"/":{"write": ".*", "read": ".*", "configure": ".*"}}'

vault read security-codedesignplus-rabbitmq/creds/ms-notification-rabbitmq-role