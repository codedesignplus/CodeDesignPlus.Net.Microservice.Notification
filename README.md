...
5. Run the desired entry point:
   ...
   - For REST API:
      ```bash
      dotnet run --project src/entrypoints/CodeDesignPlus.Net.Microservice.Notification.Rest
      ```
   ...
   - For Worker:
      ```bash
      dotnet run --project src/entrypoints/CodeDesignPlus.Net.Microservice.Notification.AsyncWorker
      ```
...
1. Build the Docker image using the Dockerfile in the REST API entry point:
   ```bash
   docker build -t ms-notification-rest . -f src/entrypoints/CodeDesignPlus.Net.Microservice.Notification.Rest/Dockerfile

   docker run -d -p 5000:5000 --network=backend -e ASPNETCORE_ENVIRONMENT=Docker --name ms-notification-rest ms-notification-rest
   ```

2. Build the Docker image using the Dockerfile in the gRPC entry point:
   ```bash
   docker build -t ms-notification-grpc . -f src/entrypoints/CodeDesignPlus.Net.Microservice.Notification.gRpc/Dockerfile

   docker run -d -p 5001:5001 --network=backend -e ASPNETCORE_ENVIRONMENT=Docker --name ms-notification-grpc ms-notification-grpc
   ```

3. Build the Docker image using the Dockerfile in the Worker entry point:
   ```bash
   docker build -t ms-notification-worker . -f src/entrypoints/CodeDesignPlus.Net.Microservice.Notification.AsyncWorker/Dockerfile

   docker run -d -p 5002:5002 --network=backend -e ASPNETCORE_ENVIRONMENT=Docker --name ms-notification-worker ms-notification-worker
   ```
